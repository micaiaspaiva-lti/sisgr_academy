...
export default function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fc] flex items-center justify-center p-4 antialiased font-sans">
      ...
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-8 md:p-10 shadow-xl flex flex-col gap-6 relative overflow-hidden">
        {/* Detalhe de Luz Decorativo */}
        <div className="absolute -top-24 -left-24 w-48 h-48 rounded-full bg-duet-brand-light/40 blur-3xl pointer-events-none"></div>
        <div className="absolute -bottom-24 -right-24 w-48 h-48 rounded-full bg-duet-brand-light/20 blur-3xl pointer-events-none"></div>

        {/* Header */}
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="bg-duet-brand-light p-3 rounded-2xl border border-duet-brand/10 text-duet-brand shadow-xs">
            <ShieldCheck className="h-8 w-8 animate-pulse" />
          </div>
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">SISGR Academy</h1>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-3xs font-extrabold text-slate-500 uppercase tracking-wider pl-1">E-mail Administrativo</label>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3.5 bg-duet-brand hover:bg-duet-brand-hover text-white rounded-xl text-xs font-bold transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-75 disabled:cursor-not-allowed cursor-pointer"
          >
